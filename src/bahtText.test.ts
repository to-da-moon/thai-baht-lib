...
describe('string', () => {
  test('should convert number in string format', () => {
    expect(bahtText(('1234' as unknown) as number)).toBe(
      'หนึ่งพันสองร้อยสามสิบสี่บาทถ้วน'
    );
    expect(bahtText(('-1' as unknown) as number)).toBe('ลบหนึ่งบาทถ้วน');
    expect(bahtText(('-30' as unknown) as number)).toBe('ลบสามสิบบาทถ้วน');
    expect(bahtText(('123456789.999' as unknown) as number)).toEqual(
      'หนึ่งร้อยยี่สิบสามล้านสี่แสนห้าหมื่นหกพันเจ็ดร้อยเก้าสิบบาทถ้วน'
    );
  });
  test('test more striing', () => {
    expect(bahtText(('01' as unknown) as number)).toBe(
      'หนึ่งบาทถ้วน'
    );
    expect(bahtText(('01.01' as unknown) as number)).toBe(
      'หนึ่งบาทหนึ่งสตางค์'
    );
    expect(bahtText(('-01' as unknown) as number)).toBe('ลบหนึ่งบาทถ้วน');
    expect(bahtText(('-01.01' as unknown) as number)).toBe('ลบหนึ่งบาทหนึ่งสตางค์');
  });
})
